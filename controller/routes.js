    ...
    con.query("select cs.*,s.region,s.vpc_id,s.instanceId,s.platform from customers cs inner join servers s on(cs.id=s.customerIAMId) where s.id = ?", [req.body.serverId], function(err, result){
        var params = {
         ...
         sts.assumeRole(params, function(err, data){
            ...
            runPatchScriptMethod(result[0],document,creds,req,res);
        });
    });   
...
app.post("/createPatchMW_NotWorking", function(req, res){
    var AWS = require('aws-sdk');
    var sts = new AWS.STS({apiVersion: '2011-06-15'});
     if(con == null)
    con = db.openCon(con);
...
function runPatchScriptMethod(result,document,creds,req,res,script){
    
    var patch_script = [];    

    patch_script[0] = 'aws ssm create-maintenance-window --name "'+req.body.patchName+'" --schedule "cron(* * * * ? *)" --duration "'+req.body.duration+'" --cutoff "'+req.body.cutOff+'" --allow-unassociated-targets --region "'+result.region+'"';
    patch_script[1] = 'aws ssm register-target-with-maintenance-window --window-id "'+req.body.WindowId+'" --resource-type INSTANCE --targets "Key=InstanceIds,Values=["'+result.instanceId+'"]" --region "'+result.region+'"';
    patch_script[2] = 'aws ssm register-task-with-maintenance-window --name "'+req.body.patchName+'" --window-id "'+req.body.WindowId+'" --targets "Key=WindowTargetIds,Values='+req.body.WindowTargetId+'" --task-arn AWS-RunPatchBaseline --service-role-arn "arn:aws:iam::622755624092:role/infraguard-switchrole" --task-type "RUN_COMMAND" --task-invocation-parameters "RunCommand={Comment=,TimeoutSeconds=60,Parameters={Operation=['+req.body.operationType+']}}" --max-concurrency "500" --max-errors "20%" --region "'+result.region+'"';
   
   console.log(patch_script[req.body.patchScript]);
    
    var script_string = patch_script[req.body.patchScript].replace(/\n/g, "##");  
    var command_array = [];
    if(req.body.runCommand==1){
        command_array = script_string.split("##");
    }else{
        if(result.platform=='Windows'){
            command_array[0] =  "echo.>infraGuard.ps1";
            command_array[1] = "attrib -r infraGuard.ps1";
            command_array[2] = "echo '"+patch_script[req.body.patchScript]+"' > infraGuard.ps1";
            command_array[3] = "./infraGuard.ps1";
        }else{
            command_array[0] = "touch /home/infraGuard.sh";
            command_array[1] = "chmod 777 /home/infraGuard.sh";
            command_array[2] = "echo '"+patch_script[req.body.patchScript]+"' > /home/infraGuard.sh";
            command_array[3] = "./home/infraGuard.sh";
            command_array[4] = "rm ./home/infraGuard.sh";
        }
    }
    ...
    ssm.sendCommand(params, function(err, data){
        if(err){ console.log(err, err.stack);}
            ...
            if( req.body.patchScript == 2 ){

                var obj = {
                    server_id : req.body.serverId,
                    patch_name:req.body.patchName,
                    window_id: req.body.WindowId,
                    task_id:req.body.WindowId,
                    target_id:req.body.WindowTargetId,
                    cron:req.body.cron,
                    duration:req.body.duration,
                    cutoff:req.body.cutOff,
                    operation_type:req.body.operationType,
